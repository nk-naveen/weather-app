...
import { haze1, rainy, snow1, sunny1 } from '../assets/backgroundImages/index';
...
export default function Weather({ weatherData, fetchWeatherData }) {
    ...
    function getBackgroundImg(weather) {
        if(weather === 'Snow') return snow1
        if(weather === 'Clear') return sunny1
        if(weather === 'Rain') return rainy
        if(weather === 'Haze') return haze1
        return haze1;   
    }

    let textColor = backgroundImage !== sunny1 ? 'white' : 'black'
